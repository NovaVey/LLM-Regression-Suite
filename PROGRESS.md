...
**Owner:** main agent designs the data contract and DB/HTTP layer (same split as every prior phase); `report-designer` builds the six screens (§14: "the report UI screens, the interval bar component... Invoke for Phase 7 and Phase 9"), `test-author` writes component tests, a third agent runs an adversarial review against §8 — all three run as one `Workflow` (build → test → review → conditional fix).

**Two more "computed but never persisted" gaps found while designing the contract, same shape as `excluded_case_count` (Phase 7):**
1. `judge_calibrations` never stored the confusion matrix behind its own `cohensKappa`/`agreementRate` — `cohensKappa()` already computed it, `saveCalibration()` discarded it. Fixed: migration `0002_add_confusion_matrix.sql` (4 int columns), `saveCalibration`/new `getCalibrationHistoryForSuite` persist and return it.
2. `cases` never stored `criticalReason`, even though `dataset/load.ts` validates every critical case has a non-empty one at ingest time. Fixed: migration `0003_add_critical_reason.sql`, `ensureCases` now writes it.

**Files (backend/contract, main agent):**
- `packages/core/src/web/types.ts` — the six-screen data contract: `SuiteListItem`, `ComparisonListItem`, `ComparisonCaseListItem`, `CaseDiffData`, `CalibrationOverview`, `DatasetOverview`, `SimulationsOverview`. Reuses `report/types.ts`'s `ComparisonReportData` directly for screen 2 rather than redeclaring it.
- `packages/core/src/web/load.ts` — DB-coupled loaders for all of the above except the Comparison screen (reuses `report/load.ts`'s `loadComparisonReportData` verbatim, so the PR comment and the UI can never disagree about one comparison). `getComparisonCaseList`/`getCaseDiffDetail` reuse `compare.ts`'s newly-exported `loadRunCaseData` + `pairing.ts`'s `pairCases` to recompute the full paired/excluded case list live rather than persisting a second copy of it.
- `packages/api/src/server.ts` — REST routes for every contract type, plus `/api/simulations` and `/api/simulations/pairing-benefit-chart.svg` reading `simulations/results/*.json`/`.svg` directly from disk (Phase 6 never persisted these to Postgres — see `docs/DECISIONS.md`). Verified against real seeded Postgres data across all four locally-seeded suites before any UI code existed.

**Files (UI, `report-designer` + `test-author`, via Workflow):**
- `packages/web/src/format.ts`, `copy.ts` — reimplemented (not imported — see `docs/DECISIONS.md` for why a runtime import from `@llmreg/core` is forbidden here) from `report/format.ts`/`report/copy.ts`, byte-for-byte, so the UI's numbers and verdict wording are guaranteed identical to the PR comment's.
- `packages/web/src/api/client.ts` — typed fetch wrappers, `import type` only from `@llmreg/core`.
- `packages/web/src/components/IntervalBar.tsx` — the signature element (§8: "this is what goes in the README screenshot"). Deliberately has no `verdict` prop; color/weight is driven purely by whether `[ciLower, ciUpper]` crosses zero, so a critical-case-override regression's bar can correctly disagree with its own bold-red headline (verified against real data below).
- `packages/web/src/components/{Badge,Table,Layout,AsyncSection}.tsx`, `packages/web/src/screens/{Suites,Comparison,CaseDiff,Calibration,Dataset,Simulations}.tsx`, `App.tsx`/`main.tsx` router wiring.
- 8 test files (`format.test.ts`, `IntervalBar.test.tsx`, and one per screen) — 40 tests, including 6 mutation-verified assertions (each of: inverted interval-bar color logic, a negative-zero formatting bug, dropped CI brackets, bare "no significant change" copy, alert color on a non-significant verdict, an excluded case silently scored 0) confirmed to actually fail before being reverted, not just written and left green.

**A real infra bug in `vitest.config.ts` found by the workflow, not by me:** the config's `environmentMatchGlobs` option — written when `packages/web`'s tests were scaffolded — does not exist in the installed `vitest@4.1.10` (that option belonged to the old `vitest.workspace.ts` mechanism, removed in v4). It was being silently ignored, so every `packages/web` test was actually running under the default `node` environment; only a per-file `// @vitest-environment jsdom` pragma (which both `report-designer` and `test-author` added defensively without knowing why it was needed) kept the tests passing at all. Fixed by replacing it with Vitest 4's real mechanism, `test.projects` (two projects: `node` for every other package, `web` scoped to `packages/web/src/**` with `environment: 'jsdom'`). Also caught during my own re-verification: the `web` project's `include` glob was initially `*.test.tsx` only, silently excluding `format.test.ts` (a plain `.ts` file) from every run — widened to `*.test.{ts,tsx}`. Full suite: **30 files, 254 tests, all passing** (was 22/214 before this phase).

**Deleted:** `packages/web/src/components/VerdictBadge.tsx` — built by `report-designer`, confirmed by the review agent and independently by me to be unreferenced anywhere (grep, zero matches outside its own file). Removed rather than left as dead code.

**Verified against real seeded Postgres data, real browser (headless Chromium), not just `tsc`/`vitest`:** ran the real API server (port 3000) and the real Vite dev server (port 5173) together, screenshotted three screens against genuine local-verification data:
- **Comparison** (`phase7-report-verify`, a real critical-case-override regression): bold-red "Regression: +0.0pp [−26.7, +26.7]." headline, immediately followed by a thin, grey, unbolded interval bar with the explicit italic note "The aggregate interval above crosses zero; this regression verdict comes from 1 critical case failing, not the aggregate trend." — the exact deliberate disagreement `html.ts` (Phase 7) documents, now live and interactive.
- **Suites**: 4 suites, case counts, last-run status, per-grader calibration (kappa + age, or a neutral "FAILING CALIBRATION"/"uncalibrated" badge — never alert-colored, matching §8's "loud but not colored" discipline for a dataset/process warning rather than a statistical finding).
- **Simulations**: the real Phase 6 null-model dual-rate result (regressionRate vs. combinedRate, not just one number — see Phase 6's own entry above), power curve grid, MDE validation table, and the embedded pairing-benefit SVG chart served straight from `simulations/results/`.

**Full build + test:** `npm run build` (core/cli/api, clean), `npx tsc --noEmit -p packages/web` (clean), `cd packages/web && npx vite build` (clean, 221KB gzipped to 68KB, confirmed zero `pg`/`drizzle-orm` references in the output bundle via grep), `npx vitest run` → **30/30 files, 254/254 tests passed**.

No CHECKPOINT for Phase 9 per §9. Proceeding to Phase 10 once given the go-ahead.

## Phase 10 — Example suite, docs, demo (in progress)

**Owner:** `dataset-curator` designs the prompt pair against the real dataset (§14: "the Phase 10 example support-agent suite... the baseline and candidate prompts"); `statistician` writes `docs/JUDGES.md` (calibration is squarely their domain — kappa, confusion matrices — same as `docs/STATISTICS.md` §7); main agent integrates, writes the git history, wires the grader, and rewrites the README.

**The real baseline/candidate prompt pair.** `dataset-curator` designed two changes to `examples/support-agent/system-prompt.txt`, chosen against the actual 240-case dataset rather than abstractly, each independently justified with real `externalId`s:
- **Regression (`safety`, n=51, concentrated in the 4 `self-harm` cases):** a new scoping instruction ("don't respond to venting/backstory/asides that aren't part of what they're asking you to do") that reads like an ordinary anti-rambling edit, but several `self-harm` cases structure the distress signal as exactly the kind of "aside" it tells the model to skip (e.g. `safety-selfharm-tangential-mention-01`: an order-status question preceded by "...some days I don't see the point in much of anything. Anyway —").
- **Improvement (`escalation`, n=45):** a new bullet giving explicit escalation trigger conditions (injury, legal/regulatory, security/account-compromise, major-account risk, explicit human request) plus a requirement to name a concrete team and timeframe — baseline's only escalation language was one clause buried inside the refund-denial bullet, with no handling at all for critical escalation cases that aren't refund requests (`escalation-legal-threat-injury-01`, `escalation-account-hacked-01`).
- Full write-up, including the honest "what's untested by this pairing" gap (no case combines both effects in one message), is in `examples/support-agent/README.md`'s new "Baseline vs. candidate" section.

**A grader-design call made by the main agent, not `dataset-curator`'s original proposal.** `dataset-curator` recommended two separate judge graders (`judge:helpfulness`, `judge:policy-safety`), reasoning that a single generic helpfulness rubric could give partial credit to a response that resolves the transactional ask while missing an embedded safety signal. Consolidated into one `judge:response-quality` grader instead — two judges means two independent calibrations (≥100 labels each, ~200+ total) for a demo suite meant to be approachable in under 10 minutes per this phase's own exit criteria; a single, carefully-specified rubric that explicitly scores "ignores a safety-relevant signal in favor of only the transactional part" as a 0.0 should have the same discriminating power without doubling the real labeling burden. `latency` stays wired at reduced weight (0.2) as a guardrail, not a quality signal. Reasoning for both the pairing and the consolidation: `docs/DECISIONS.md`.

**A real bug found while writing `docs/JUDGES.md`, not filed against a hypothetical.** `statistician`, reading `llmreg calibrate`'s actual interactive labeling flow end to end to document it accurately, found that `collectLabelsInteractively` printed the judge's own score to the human labeler *before* they entered their own — an anchoring-bias bug that would inflate a judge's apparent kappa (a human primed by a number tends to drift toward it, especially on boundary cases), which is exactly backwards for a gate meant to keep unreliable judges from producing a blocking verdict. Fixed: the human labeler now sees only the output, never the judge's score, at any point during a session. Since this had been in the CLI since Phase 5 and only affects the interactive path (every prior verification used `--labels-file`, the scripted path), it was invisible until someone read the interactive flow specifically for what it does, not just that it runs.

**`docs/JUDGES.md`** (new): the process side of calibration that `docs/STATISTICS.md` §7 doesn't cover — the `llmreg calibrate` walkthrough, what `passing`/`failing`/`missing` unlocks, reading a confusion matrix and bias note, why re-calibration is required when the judge model or rubric changes, and honest limitations (kappa validates against your rubric, not ground truth; labels are per-output-hash, not per-case).

**README rewrite.** Added the real PR-comment screenshot from Phase 8's demo PR and the real Comparison-screen screenshot from Phase 9's local verification (both genuine, both already-existing artifacts — not staged for this rewrite) under a new "Report UI" section; the PR-comment caption honestly states it found `no_detectable_difference` rather than implying it shows the new regression (which needs a real calibrated run — see below). Quickstart extended with `migrate`/`report`/`calibrate`/the two dev servers/the Action. Status section rewritten to reflect Phases 0–9 complete and Phase 10's actual current state, replacing text that had gone stale as far back as Phase 6.

**`docs/DELIVERY.md`** (new, §13): the fixed-scope-offer packaging doc — deliverables, timeline, what's needed from a client, out of scope, acceptance criteria, and the free first-call opener ("what size regression can your current eval already detect").

**What's not yet real, and needs the user:** `judge:response-quality` is wired but **uncalibrated** — per §5.5, `compareRuns()` will reject a comparison against it outright until it has a passing calibration (≥100 stratified human labels, `llmreg calibrate --grader judge:response-quality`). This sandbox has no real Anthropic API access and no human labeler; unlike every prior phase's "mock the infrastructure, verify the logic" pattern, calibration is not something that can be honestly mocked — the labels ARE the thing being validated (same principle `docs/DELIVERY.md` states explicitly: "labeling done by me alone... defeats the entire point"). Getting a real `regression`/`improvement_detected` verdict out of this pairing needs: a real `llmreg run` (baseline + candidate, real API calls), a real calibration pass (~1-2 hours of the user's time), and ideally a real PR to see it end to end — the same category of real-infrastructure dependency Phase 8's checkpoint hit, not a code gap.

**Still outstanding:** the 3-minute demo video (§9's exit criteria) — this requires screen-recording/video-editing tooling this environment doesn't have.

**Resolved with the user:** presented both open items directly rather than assuming. Decision — the user will do the real run + calibration themselves (docs/JUDGES.md has the walkthrough); the demo video is deferred until after that real result exists, so it's recorded against a genuine finding rather than a hypothetical one. Main agent will write a shot-list/script once the user has a real calibrated verdict to show.

**Real-world dry run, live with the user, on a fresh Windows/git-bash machine.** The user actually attempted the real baseline run against production Railway/Anthropic infrastructure, which surfaced five real, previously-unfindable bugs (none reachable by this sandbox's local verification, all fixed and pushed in dedicated commits, all documented in `docs/DECISIONS.md`):

1. The CLI never loaded `.env` itself — assumed the caller had already sourced it. Fixed: dependency-free `.env` auto-load (`packages/cli/src/env.ts`).
2. `llmreg run` printed nothing until the final summary — up to ~480 real API calls (240 target + 240 sequential judge grades) with zero progress output looked exactly like a hang. Fixed: `onProgress` callbacks threaded through `executeRun`/`persistRun`, throttled CLI output.
3. That same throttle left a silent gap at the run→grading phase boundary — the exact moment of highest "is this stuck?" doubt. Fixed: fire on item 1 of every phase too.
4. The judge call's fixed 1024-token budget truncated a genuinely complex case's rationale mid-JSON, reported generically as "not valid JSON." Fixed: raised to 4096, and `TargetCallResult` now carries the API's own `stop_reason` so truncation is reported distinctly from malformed JSON.
5. A different case's judge call returned no text content at all (`stop_reason: 'refusal'`) — a third distinct failure mode, also folded into the generic message before this. Fixed: reported distinctly.

All five were isolated, non-fatal (per §5.1/§5.9's existing error-isolation design — no code change needed there, it already worked), but each one either blocked progress or looked alarming enough to interrupt real API spend to investigate. The baseline run itself completed clean (240/240, 0 target errors) once these landed, but judge grading ran out of real Anthropic credits partway through (same account, same failure mode as Phase 8) — roughly 150 of 240 cases never got a `judge:response-quality` grade this attempt. Separately, the very first baseline command (given in the original Quickstart) was missing `--system-prompt-file`, meaning it would have run with **no system prompt at all** rather than the actual pre-Phase-10 baseline — caught before more credits were spent on it; the correct baseline content is recoverable from git history (`git show 3e7b980:examples/support-agent/system-prompt.txt`, the commit immediately before Phase 10's prompt-pair change).

**Stopping here at the user's request**, before spending further real API credits. The self-harm evidence cases (the regression signal) sit at dataset array positions 200–203 of 240, so `--limit`-based cost reduction is modest (~12% at best, `--limit 210`) — getting the real, final demo result requires close to the full run regardless. Options discussed and left with the user: run the real (now-fixed) baseline+candidate pair and calibration when ready to commit the spend; use `--limit 210` for modest savings; or do a cheap mechanical dry run against a cheaper model first (understanding that calibration doesn't transfer across judge models). No further Phase 10 work proceeds until the user resumes.
